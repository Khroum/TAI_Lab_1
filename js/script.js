...
function removeMark(elem, isCorrect) {
  elem.classList.remove(isCorrect);
}
...
function doAction(event) {
  ...
  if (event.target.innerHTML === preQuestions[index].correct_answer) {
    ...
    currentAnswer = { event: event.target, correct: "correct" };
    ...
    currentAnswer = { event: event.target, correct: "incorrect" };
    answeredQuestions.push({
      index: index,
      event: event.target,
      correct: "incorrect"
  ...
  if (answeredQuestions.length === preQuestions.length) {
    saveData();
    removeMark(currentAnswer.event, currentAnswer.correct);
    list.style.display = "none";
    results.style.display = "block";
    userScorePoint.innerHTML = points;
    average.innerHTML = localStorage.getItem("points");
  }
...
function saveData() {
  ...
  if (data === null) {
    ...
    let games = parseInt(localStorage.getItem("games")) + 1;
    data = parseInt(data) + points;
    localStorage.setItem("points", data / games);
  ...
  if (index < preQuestions.length - 1) {
    ...
    answeredQuestions.forEach(element => {
      if (element.index == index) {
        isAnswered = true;
        currentAnswer = { event: element.event, correct: element.correct };
        if (element.correct == "correct") {
          markCorrect(element.event);
        } else {
          markInCorrect(element.event);
        }
      }
    });
